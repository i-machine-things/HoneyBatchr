...
## 2026-05-09 — `modules/updater.py` + `modules/_version.py` (PR #13 polish — feat/update-checker)

**Review:** Claude code review of update checker — 4 findings.
**Result:** All 4 fixed in commit 6be9391.

### Findings

1. **`_dns_a()` missing bounds check before question-section skip**
   - `while resp[pos]` had no guard; a truncated DNS response raises `IndexError` (swallowed by caller but silently kills the fallback)
   - Fix: change to `while pos < len(resp) and resp[pos]`; add `if pos + 5 > len(resp): return []` before advancing past the question section

2. **`UpdateDownloader` created without a Qt parent**
   - `super().__init__()` called with no parent, inconsistent with `UpdateChecker(parent)`; object not in Qt ownership tree
   - Fix: add `parent=None` parameter, pass to `super().__init__(parent)`; call site updated to `UpdateDownloader(..., self)`

3. **Lazy standard-library imports inside `UpdateDialog.__init__`**
   - `import platform`, `import html`, `import webbrowser` deferred inside `__init__` and method bodies with no benefit; adds noise and obscures dependencies
   - Fix: move all three to module-level imports

4. **`_version.py` "dev" sentinel undocumented**
   - No indication that `"dev"` is intentional and stamped by the release workflow; looks like a forgotten placeholder
   - Fix: add one-line comment explaining the CI stamp expectation
